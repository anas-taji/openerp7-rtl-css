...
class Home(openerpweb.Controller):
    ...
    def index(self, req, s_action=None, db=None, **kw):
        ...
        # here, we need to figure out what is the current lang
        # from req.session.get_context(), then 
        # fetch lang (direction) attributes to see if its RTL
        # and add special class "rtl" on body tag.
        # the http.HttpRequest class has been hacked
        # to load session from cookie if its available.
        # then use it here, before that this request 
        # has no session at all, even if the user already
        # logged in.
        # issue: login / logout actions dosn't make 
        # full page reload. so, after you login you need 
        # to hit browser refresh in order to see correct 
        # page direction, this is not the case with 
        # user Preference wizard, which leads in full page 
        # reload after save action.
        lang = self.get_lang(req)
        is_rtl = lang.get('direction', 'ltr') == 'rtl'
        r = html_template % {
            'js': js,
            'css': css,
            'is_rtl': is_rtl and 'rtl' or '',
            'css_rtl': is_rtl and '<link rel="stylesheet" href="/web/static/src/css/rtl.css" />' or '',
